...
export default function AuthForm({ authType }: AuthFormProps) {
  ...
  const showToast = (title: string, description: string, variant : 'destructive' | 'default' = 'destructive', action?: ToastActionElement | undefined) => {
  ...
  const handleLogin = async ():Promise<boolean> => {
    ...
    if (!doesEmailExist.success) {
      showToast('Error Logging in', doesEmailExist.message, 'destructive',
  ...
  try {
    ...
    const isUserValid = authType === 'sign-up' ? await handleSignUp() : await handleLogin();
    ...
    if (result?.error) {
      ...
      showToast('Check Your Email', 'We have sent a magic link to your email. Click the link to complete your login.', 'default');
    }
  } catch (error) {
  ...
  return (
      <form onSubmit={handleSubmit}>
        ...
        <div className="space-y-4 mt-5">
          ...
          >
            {isLoading ? (<RotateCw className='animate-spin' />): (authType === 'sign-up' ? 'Sign Up' : 'Log in')}
          </Button>
        </div>
      </form>
  );
